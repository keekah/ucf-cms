...
from tutorial.quickstart.models import Project, Student, StudentProjectRanking, Schedule, ScheduleProject, ScheduleStudent, CMSProject, CMSMembers, FinalSchedule
...
from tutorial.quickstart.helpers import GetCurrentTerm, ConvertAlgFileToJson, BuildInitialGrid, RunBefore, ConvertGridJsonToAlgJson
...
def CreateCmsProject(request):
    try:
    #design_doc_url = models.CharField(max_length=100)
    #final_doc_url = models.CharField(max_length=100)
    #presentation_url = models.CharField(max_length=100)
    #conference_paper_url = models.CharField(max_length=100)
        data = json.loads(request.body)
...
def GetGroups(request):
    try:
        listFileString = "tutorial/quickstart/Scheduler/Schedule_Runs/{}/{}/{}".format(2019, 'summer', '07-25-2019_17_43_12.txt')
        try:
            data = json.loads(request.body)
            year = data['year']
            #reanem to term
            term = data['ter']
            finalObj = FinalSchedule.objects.get(Year=year, Term=term)
            listFileString = "tutorial/quickstart/Scheduler/Schedule_Runs/{}/{}/{}".format(year, term, version)
            currentRun = ConvertAlgFileToJson(listFileString, year, term)
        except:
            try:
                datum = ConvertAlgFileToJson(listFileString, '2019', 'summer')
                currentRun = json.loads(datum)
            except:
                return HttpResponse(status=400, content=json.dumps("No schedule loaded"))
        groupList = []
...
def FinalizeScheduleVersion(request):
    try:
        ...
        if(len(query2) > 0):
            query2 = FinalSchedule.objects.get(Year=year, Term=term, ScheduleVersion=version)
